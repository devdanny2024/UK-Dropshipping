...
## Data Flow
1) Client or Admin posts a product URL to `/v1/resolve-product` to create a `ProductSnapshot`. The backend enqueues a `resolveProduct` job which fetches the upstream store page (ASOS, Zara, Amazon UK, Nike, H&M, etc.) and updates the snapshot with real product metadata (title, image, price, currency).
